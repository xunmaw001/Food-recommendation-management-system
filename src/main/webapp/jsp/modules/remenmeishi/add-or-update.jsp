...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑热门美食</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>热门美食管理</span></li>
								<li class="breadcrumb-item active"><span>编辑热门美食</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">热门美食信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>材料</label>
										<input id="cailiao" name="cailiao" class="form-control" placeholder="材料">
									</div>
										
											<div class="form-group  col-md-12">
												<label>美食特色</label>
												<div><div class="upload"><input id="meishiteseupload" name="file" type="file">选择文件</div>
												<script id="meishiteseEditor" type="text/plain" style="width:800px;height:230px;"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var meishiteseUe = UE.getEditor('meishiteseEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="meishitese" id="meishitese-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "remenmeishi/info/" + id,
		...
		function upload(){
			...
			$('#meishiteseupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('meishiteseEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("remenmeishi/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.meishitese != null && ruleForm.meishitese != 'null' && ruleForm.meishitese != ''){
				var meishiteseMes = '' + ruleForm.meishitese;
				var meishiteseUeditor = UE.getEditor('meishiteseEditor');
				meishiteseUeditor.ready(function() {
					meishiteseUeditor.setContent(meishiteseMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('meishiteseEditor').hasContents()){
				$('#meishitese-input').attr('value',UE.getEditor('meishiteseEditor').getPlainTxt());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("remenmeishi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.cailiao != null && res.data.cailiao != ''){

					$("#cailiao").val(res.data.cailiao);
					}
